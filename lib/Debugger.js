...
function getRelativePath(file) {
    ...
    return file.replace(currentRoot, '').split(separator).join('/');
}
...
export default class Debugger {
    ...
    toggleBreakpoint(editor, file, line) {
        if (file && (file.indexOf(['cartridge', 'controllers'].join(separator)) !== -1 ||
          file.indexOf(['cartridge', 'scripts'].join(separator)) !== -1)) {
