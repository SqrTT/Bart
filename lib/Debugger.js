...
export default class Debugger {
    ...
    setActiveBreakpoint(maybeThread) {
        ...
        if (maybeThread && maybeThread.threadInfo.call_stack) {
            ...
            const dwPath = maybeThread.threadInfo.call_stack[0].location.script_path;
            atom.workspace.open(workspacePath + dwPath, {
                initialLine: maybeThread.threadInfo.call_stack[0].location.line_number - 1
            }).then(editor => {
                this.activeBreakpointMarker = editor.markBufferPosition(
                    [maybeThread.threadInfo.call_stack[0].location.line_number - 1, 0], {invalidate: 'never'});
    ...
    update() {
        this.connectionInstanse.getThreads().then((threads) => {
            ...
            if (this.currentThread) {
                this.debuggerView.updateValues(null);
                this.currentThread.getMembers().then((values) => {
