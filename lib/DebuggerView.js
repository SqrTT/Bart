...
const StackView = React.createClass({
    render: function () {
        if (this.props.thread === 'spin') {
            return <span className='loading loading-spinner-small inline-block'></span>;
                    ...
                    <ul className='list-group'>
                        {this.props.thread.threadInfo.call_stack.map((stack, index) =>
                            <li
                                ...
                                onClick={this.props.onSelectFrame ? this.props.onSelectFrame.bind(this, index) :  noop}
...
const VarView = React.createClass({
    ...
    toggleOpened: function () {
        ...
        if (!this.state.values && this.props.thread) {
            ...
            this.props.thread.getMembers(this.props.path, this.props.selected).then(function (data) {
                ...
                (<ul className='list-tree'>
                    {this.state.values.map(value =>
                        <li className="list-nested-item">
                            <VarView type={value.type} value={value.value} name={value.name} path={this.props.path + '.' + value.name}
        ...
        return (
            <div className="bart_pad_tree">
                <div className='list-item'>
                    <span className={this.state.opened ? 'icon icon-chevron-down' : 'icon icon-chevron-right'}
