...
class WebDav {
    ...
    unzipRx (filePath, root = this.config.root) {
        ...
        return this.makeRequestRx({
            ...
            console.log('unzipRx-response', data);
        });
    }
    ...
    deleteRx(filePath, optionalRoot) {
        ...
        return Observable.create(observer => {
            console.log('deleteReuest', uriPath);
            const req = request(Object.assign(this.getOptions(), {
                uri: '/' + uriPath,
                method: 'DELETE'
            }), (err, res, body) => {
    ...
    uploadCartridgesRx (
        ...
        const processingFolder = pathToCartridgesDir.split(path.sep).pop();
        const cartridgesZipFileName = path.join(pathToCartridgesDir, 'cartridges.zip');


        return this.deleteLocalFileRx(cartridgesZipFileName)
            .do(() => {
                notify(`[${processingFolder}] Deleting local zip`);
            })
            .flatMap(() => {
                notify(`[${processingFolder}] Zipping`);
                return this.zipFilesRx(pathToCartridgesDir, cartridgesZipFileName, options)
            })
            .flatMap(() => {
                notify(`[${processingFolder}] Deleting remote zip`);
                return this.deleteRx(cartridgesZipFileName, pathToCartridgesDir)
            })
            .flatMap(() => {
                notify(`[${processingFolder}] Sending zip to remote`);
                return this.postRx(cartridgesZipFileName, pathToCartridgesDir)
            })
            .flatMap(() => {
                notify(`[${processingFolder}] Deleting local zip...`);
                return this.deleteLocalFileRx(cartridgesZipFileName);
            ...
            .flatMap(() => {
                ...
                return this.unzipRx(cartridgesZipFileName, pathToCartridgesDir)
            });
    }
    uploadCartridges (
        pathToCartridgesDir,
